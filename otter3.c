...
#define PROMISC 1
#define Q_SIZE = 250
char *db = NULL;	//global db variable
...
int main(int argc, char *argv[]) { //arguments are for later...
   ...
   char *intface = NULL, *filter = NULL; // capturing interface, db_file, and pcap_filter
   ...
   if(argc <= 1) {
      ...
      for(num_options=1; num_options < argc; num_options++) {
		 ...
		 else if(strcmp(argv[num_options],"-")==0)
			printf("Usage: -d \"db file\" -i \"interface\" -h \"help\"\n\n");
	  }
   }
...
/* get network number and mask associated with capture device */
   ...
   pcap_loop(pstream, 10, &parser, NULL); 
   //for completeness
   return 0;
}
...
// int size_tcp;
//sig
//   int wss, ttl, mss, df, origw, sok, nop, origtot;
//   void* opt_ptr;
//endsig
   int i;
   struct fprint fp;
...
// Print this shit
   ...
   //printf(":%d:%d:%d:",fp.ittl,df,fp.tot);
   ...
   if (!fp.quirks) { sprintf(fingerprint + strlen(fingerprint), "."); }
      ...
      if (fp.quirks & QUIRK_ACK) { sprintf(fingerprint + strlen(fingerprint), "A"); packet_type=1; }	//syn=0, ack=1, syn-ack=2
...
//   size_ip = 10;
   ...
   */
   ...
   // convert ip and mac to type string
   //snprintf(dhost_mac, sizeof dhost_mac, "%02X:%02X:%02X:%02X:%02X:%02X",ethernet->dhost[0],ethernet->dhost[1],ethernet->dhost[2],ethernet->dhost[3],ethernet->dhost[4],ethernet->dhost[5]);
   ...
   //snprintf(dhost_ip, sizeof dhost_ip, "%d.%d.%d.%d",internet->daddr[0],internet->daddr[1],internet->daddr[2],internet->daddr[3]);
...
int sql_query(char host_ip[20], char host_mac[20], int packet_type, char fingerprint[40])
{
   ...
   int retval=0;
   //# of queries, size of each query and pointer
   //int q_cnt = 5;
   ...
   char create_table[250] = "CREATE TABLE IF NOT EXISTS prints (sip TEXT ,smac TEXT,syn_ack INTEGER,fingerprint TEXT)";
   char create_table2[250] = "CREATE TABLE IF NOT EXISTS mismatches (sip TEXT ,smac TEXT,syn_ack INTEGER,fingerprint TEXT,mismatch_code INT)";
   
   //execute query to create prints table
   retval = sqlite3_exec(handle,create_table,0,0,0);
   if(retval)
   {
      printf("Error while creating prints table\n");
      return -1;
   }
   ...
   // check whitelist
   char query[250];
   snprintf(query, sizeof query, "SELECT * FROM prints WHERE (sip=\'%s\' OR smac=\'%s\') AND syn_ack=\'%d\'",host_ip, host_mac, packet_type);
   //printf("Full Query: %s\n",query);
   ...
   while(run) {
      ...
      if(retval == SQLITE_ROW) {
            ...
            switch (col){
               case 0:
                   if(strcmp(host_ip,val) == 0)
                      printf("Host IP's match\n");
                   else {
                      printf("Host Ip has changed\n");
                      mismatch+=1;
                   }
               break;
               case 1:
                  if(strcmp(host_mac,val) == 0) 
                     printf("Host Mac's match\n");
                  else {
                     printf("Host Mac has changed\n");
                     mismatch+=2;
                  }
               break;
               case 2:
                  if(packet_type == *val)
                     printf("Packet Types match\n");    //probably don't need this
               break;
               case 3:
                  if(strcmp(fingerprint,val) == 0)
                     printf("Fingerprints match\n");
                  else {
                     printf("WARING!!! Fingerprint has changed!\n");
                     mismatch+=4;
                  }
               break;
               default:
         ...
         if(rows == 0)
            mismatch=7;
            //printf("Mismatch: %d\n",mismatch);    //if no mismatch, mismatch=0    if not found in table, mismatch=7
         if(mismatch > 0) {
	    printf("Not on white-list!\n");
            ...
            if(retval == SQLITE_ROW) {
               ...
               if(*val == '0') {
                  ...
                  snprintf(query, sizeof query, "INSERT INTO mismatches VALUES('%s','%s','%d','%s','%d')",host_ip, host_mac, packet_type, fingerprint, mismatch);
                  //printf("Full Query: %s\n",query);
